...
comp_G = 0x036B17D1F2E12C4247F8BCE6E563A440F277037D812DEB33A0F4A13945D898C296
...
Point = Tuple[int, int]
SchnorrSign = Tuple[bytes, bytes] # (sigma, aux_rand)
...
def get_random_number():
    ...
    rnd_big = int(random.getrandbits(bit_length - 1))
    if rnd_big == 0:
        rnd_big = 1
...
"""def schnorr_sign(msg: bytes, seckey: bytes, aux_rand: bytes) -> bytes:
    ...
    t = xor_bytes(bytes_from_int(d), hash_sha256(aux_rand))
    k0 = int_from_bytes(hash_sha256(t + bytes_from_point(P) + msg)) % n
    if k0 == 0:
...
"""def schnorr_sign(msg: bytes, seckey: bytes, aux_rand: bytes) -> bytes:
    hashed = hash_sha256(msg + aux_rand)
    sigma = point_add(seckey, point_mul(hashed, seckey))
    return SchnorrSign(sigma, aux_rand)

def schnorr_verify(msg: bytes, pubkey: bytes, sig: SchnorrSign) -> bool:
    hashed = hash_sha256(msg + pubkey)
    left_side = point_mul(comp_G, sig[0])
    right_side = point_add(sig[1], point_mul(pubkey, msg))"""
def schnorr_sign(msg: bytes, seckey: bytes, aux_rand: bytes) -> bytes:
    return msg
